...
class Assumption:
    ...
    def create_assumption_df(self):
        ...
        '''
        scenario_assumptions_query = "select * from hive_metastore.federated_postgres.federated_optimisations_assumptions where scenario = '" + self.scenario.name + "' order by period_start asc"
        ...
        df_assumptions_num = df_assumptions_num.pivot(index='period_start',columns='identifier', values='numeric_value').fillna(method='ffill').fillna(value=0)
    ...
    def filter_df(self):
        ...
        df_filter = self.df_num
        if self.scenario.start != None:
            df_filter = self.df_num.loc[self.scenario.start:] 
        if self.scenario.end != None:
            df_filter = self.df_num.loc[:self.scenario.end]  
        self.df = df_filter
